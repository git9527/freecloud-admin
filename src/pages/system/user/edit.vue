<template>
  <view class="uni-container">
    <uni-forms ref="form" v-model="formData" :rules="rules" validateTrigger="bind" @submit="submit">
      ...
      <uni-forms-item name="site" label="绑定站点">
        <uni-data-checklist v-if="sites.length" :value="formData.site" :range="sites" @change="binddata('site', $event.detail.value)"></uni-data-checklist>
        <view v-else class="uni-form-item-empty">
          暂无
        </view>
      </uni-forms-item>
  ...
  methods: {
    ...
    getDetail (id) {
      ...
      db.collection(dbCollectionName)
        .doc(id)
        .field('username,role,mobile,email,status,site')
        .get()
        .then((res) => {
    ...
    loadSites () {
      db.collection('opendb-netdisk-sites').get().then(res => {
        this.sites = res.result.data.map(item => {
          return {
            value: item._id,
            text: item.htmlTitle
          }
